...
function importFromRepo<T = unknown>(repoRelativePath: string): Promise<T> {
  const url = pathToFileURL(join(process.cwd(), repoRelativePath)).href;
  // biome-ignore lint/suspicious/noExplicitAny: dynamic import by file URL
  return import(url) as any as Promise<T>;
}
...
async function updateReadme(): Promise<void> {
  ...
  await importFromRepo("plugins/index.ts");

  // Now import the registry utilities
  const { getAllIntegrations } =
    await importFromRepo<RegistryModule>("plugins/registry.ts");
...
async function generateTypesFile(): Promise<void> {
  ...
  // Combine plugin types (directory names) with system types.
  // This avoids relying on runtime registry state (which may differ depending on import resolution).
...
async function processStepFilesForCodegen(): Promise<void> {
  const { getAllIntegrations, computeActionId } =
    await importFromRepo<RegistryModule>("plugins/registry.ts");
  const integrations = getAllIntegrations();
...
async function generateStepRegistry(): Promise<void> {
  ...
  const { LEGACY_ACTION_MAPPINGS } =
    await importFromRepo<LegacyMappingsModule>("plugins/legacy-mappings.ts");
  const integrations = getAllIntegrations();
